...
class EgoEmgMemmapDataset(Dataset):
    ...
    emg_layout: str = "bilateral"
    emg2pose_channel_indices_1based: Sequence[int] | None = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.emg_field_preference not in {"raw", "filtered"}:
            raise ValueError(
                f"emg_field_preference must be one of raw/filtered, got {self.emg_field_preference}"
            )
        if self.emg_layout not in {
            ...
            raise ValueError(
                ...
                "bilateral/target_hand/emg2pose_sparse16/emg2pose_interpolate16, "
    ...
    def _resolve_emg2pose_channel_indices(self) -> np.ndarray | None:
        if self.emg2pose_channel_indices_1based is None:
            return None
        indices = np.asarray(self.emg2pose_channel_indices_1based, dtype=np.int64) - 1
        if indices.ndim != 1:
            raise ValueError("emg2pose_channel_indices_1based must be a 1D sequence")
        if np.any(indices < 0) or np.any(indices >= 16):
            raise ValueError(
                f"emg2pose_channel_indices_1based out of range: {self.emg2pose_channel_indices_1based}"
            )
        return indices
    ...
    def _load_norm_stats(self) -> None:
        ...
        if item is None:
            raise KeyError(
                f"Dataset normalization stats for '{self.dataset_name}' not found in {self.norm_stats_path}"
            )
    ...
    def _prune_hand_fields(
        ...
        *,
        ...
        # Bilateral EMG layout needs both hands' EMG
        if emg_layout == "bilateral":
            needed |= {
                f"emg_{target_hand}_raw",
                f"emg_{target_hand}_filtered",
                f"emg_{other_hand}_raw",
                f"emg_{other_hand}_filtered",
            }
    ...
    def _resolve_requested_fields(self) -> tuple[set[str], set[str]]:
        ...
        wanted.update(self.fields or [])

        unknown = wanted - frame_fields - episode_fields
        if unknown:
            raise ValueError(f"Unknown memmap field(s): {sorted(unknown)}")
    ...
    def _convert_to_emg2pose_layout(
        ...
        *,
        ...
        if self._emg2pose_channel_indices is None:
            raise ValueError(
                "emg2pose alignment requires emg2pose_channel_indices_1based to be provided"
            )
        aligned = emg
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if self.target_hand is not None:
            ...
            if self.skip_emg_loading:
                ...
                if self.emg_layout == "bilateral":
                    left_key = f"emg_left_{self.emg_field_preference}"
                    right_key = f"emg_right_{self.emg_field_preference}"
                    emg = np.concatenate(
                        [
                            np.asarray(result[left_key], dtype=np.float32),
                            np.asarray(result[right_key], dtype=np.float32),
                        ],
                        axis=1,
                    )
