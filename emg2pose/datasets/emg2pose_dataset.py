...
class Emg2PoseDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        emg = self._apply_norm(emg, user_id=user_id, side=side)

        finite_mask = torch.isfinite(joint_angles).all(dim=1)
        if not finite_mask.all():
            joint_angles = torch.nan_to_num(
                joint_angles, nan=0.0, posinf=0.0, neginf=0.0
            )

        mask = torch.as_tensor(window["valid_mask"], dtype=torch.bool)
